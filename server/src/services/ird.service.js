...
export async function getIrdVatSummary(monthStr) {
  ...
  return {
    ...
    summary: {
      sales: {
        taxable: salesTaxablePaisa,
        vat: salesVatPaisa,
        gross: salesGrossPaisa,
        taxableNpr: Math.round(salesTaxablePaisa / 100),
        vatNpr: Math.round(salesVatPaisa / 100),
        grossNpr: Math.round(salesGrossPaisa / 100),
        count: salesRegister.length
      },
      creditNotes: {
        taxable: creditNotesTaxablePaisa,
        vat: creditNotesVatPaisa,
        gross: creditNotesGrossPaisa,
        taxableNpr: Math.round(creditNotesTaxablePaisa / 100),
        vatNpr: Math.round(creditNotesVatPaisa / 100),
        grossNpr: Math.round(creditNotesGrossPaisa / 100),
      ...
      purchases: {
        taxable: purchasesTaxablePaisa,
        exempt: purchasesExemptPaisa,
        vat: purchasesVatPaisa,
        gross: purchasesGrossPaisa,
        taxableNpr: Math.round(purchasesTaxablePaisa / 100),
        exemptNpr: Math.round(purchasesExemptPaisa / 100),
        vatNpr: Math.round(purchasesVatPaisa / 100),
        grossNpr: Math.round(purchasesGrossPaisa / 100),
        count: purchaseRegister.length
      },
      debitNotes: {
        taxable: debitNotesTaxablePaisa,
        vat: debitNotesVatPaisa,
        gross: debitNotesGrossPaisa,
        taxableNpr: Math.round(debitNotesTaxablePaisa / 100),
        vatNpr: Math.round(debitNotesVatPaisa / 100),
        grossNpr: Math.round(debitNotesGrossPaisa / 100),
        count: debitNotesRegister.length
      },
      netSalesVat: netOutputVatPaisa,
      netPurchasesVat: netInputVatPaisa,
      netVatPayable: netVatPayablePaisa,
      netSalesVatNpr: Math.round(netOutputVatPaisa / 100),
      netPurchasesVatNpr: Math.round(netInputVatPaisa / 100),
      netVatPayableNpr: Math.round(netVatPayablePaisa / 100),
      netVatPayablePaisa
    },
    // Statutory Registers
    salesRegister,
